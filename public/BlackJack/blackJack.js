...
async function getChips() {
  chipTotal = await fetch("http://localhost:5000/BlackJack/chipAmount")
    .then((res) => res.json())
    .then((data) => data.data);
  chipFormat();
  return chipTotal;
...
function chipFormat() {
  ...
  if (chipTotal < 5000) {
    document.getElementById("pinkChip").style.visibility = "hidden";
    moveButtonLeft = 1;
  }
  if (chipTotal < 1000) {
    document.getElementById("yellowChip").style.visibility = "hidden";
    moveButtonLeft = 2;
  }
  if (chipTotal < 500) {
    document.getElementById("purpleChip").style.visibility = "hidden";
    moveButtonLeft = 3;
  }
  if (chipTotal < 100) {
    document.getElementById("blackChip").style.visibility = "hidden";
    moveButtonLeft = 4;
  }
  if (chipTotal < 25) {
    document.getElementById("greenChip").style.visibility = "hidden";
    moveButtonLeft = 5;
  }
  if (chipTotal < 5) {
    document.getElementById("redChip").style.visibility = "hidden";
    moveButtonLeft = 6;
  }
  if (chipTotal < 1) {
    document.getElementById("blueChip").style.visibility = "hidden";
    moveButtonLeft = 7;
  }
  //move button left depending on how many chips are showing
  document.getElementById("PlaceBetButton").style.right =
    moveButtonLeft * 87 + "px";
...
function playerHitCard(event) {
  ...
  //hide split button.
  button.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling.style.visibility =
    "hidden";
...
function playerSplitCard() {
  if (playerFirstCard.value === playerSecondCard.value && splitCount === 0) {
    ...
    playerSecondHand.childNodes[3].childNodes[1].childNodes[0].id =
      "playerSecondHandFirstCard";
    playerSecondHand.childNodes[3].childNodes[3].childNodes[0].id =
      "playerSecondHandSecondCard";
    playerSecondHand.childNodes[5].style.visibility = "hidden";
    //add event listners for next hand buttons.
    playerSecondHand.childNodes[5].childNodes[1].addEventListener(
      "click",
      playerHitCard
    );
    playerSecondHand.childNodes[5].childNodes[3].addEventListener(
      "click",
      playerStay
    );
    playerSecondHand.childNodes[5].childNodes[5].addEventListener(
      "click",
      playerDoubleDown
    );
    ...
    document.getElementById("playerSecondCard").src = getCardImageLink(
      handOneNewCard.suite,
      handOneNewCard.value
    );
    document.getElementById("playerSecondHandFirstCard").src =
      document.getElementById("playerSecondHandSecondCard").src;
    document.getElementById("playerSecondHandSecondCard").src =
      getCardImageLink(handTwoNewCard.suite, handTwoNewCard.value);

    //setting scores to correct values
    playerTotal =
      playerTotal / 2 + valueParser(handOneNewCard.value, true, true);

    if (playerSecondCard.value === "A") {
      amountOfAces--;
    }
...
function playerStay(event) {
  ...
  event.target.nextSibling.nextSibling.nextSibling.nextSibling.style.visibility =
    "hidden";
...
function DealersTurn() {
  ...
  if (!secondHand) {
    ...
    document.getElementById("dealerFirstCard").src = getCardImageLink(
      dealerFirstCard.suite,
      dealerFirstCard.value
    );
    ...
    while (dealerTotal < 17) {
      ...
      if (dealerTotal > 21 && dealersAces > 0) {
        dealerTotal -= 10;
        dealersAces--;
      }
    }
  ...
  //when player splits hand
    ...
    document.getElementById("playerSecondHand").childNodes[5].style.visibility =
      "visible";
    ...
    var linkLength = document.getElementById("playerSecondHandFirstCard").src
      .length;
    ...
    playerTotal =
      valueParser(
        document
          .getElementById("playerSecondHandFirstCard")
          .src.substring(linkLength - 6, linkLength - 5),
        true,
        true
      ) +
      valueParser(
        document
          .getElementById("playerSecondHandSecondCard")
          .src.substring(linkLength - 6, linkLength - 5),
        true,
        true
      );
...
function gameEnd() {
  ...
  if (getValueOfBothHands) {
    ...
    if (
      (playerFirstHandTotal > dealerTotal && playerFirstHandTotal < 22) ||
      (dealerTotal > 21 && playerFirstHandTotal < 22)
    ) {
      if (
        document.getElementById("playerCardList").childElementCount === 2 &&
        playerFirstHandTotal === 21
      ) {
        totalBet *= 2.5;
      } else {
        totalBet *= 2;
    ...
    if (
      ...
      if (
        document.getElementById("playerSecondHand").childNodes[3]
          .childElementCount === 2 &&
        playerTotal === 21
      ) {
        totalBet *= 2.5;
      ...
      totalBet = 0;
      document.getElementById("playerSecondHand").childNodes[7].style.color =
        "red";
    }

    document.getElementById("playerSecondHand").childNodes[7].innerText =
      sign + " $" + totalBet;
  ...
  //Post player Chips to Server
  fetch("http://localhost:5000/BlackJack/chipAmount", {
    method: "POST",
    headers: {
  ...
  document
    .getElementById("newHandButton")
    .addEventListener("click", gameTransition);
}
...
function gameTransition() {
  ...
  while (
    document.getElementById("dealerSecondCard").parentElement != lastCard
  ) {
  ...
  while (
    document.getElementById("playerSecondCard").parentElement != lastCard
  ) {
    lastCard = lastCard.previousSibling;
    lastCard.nextSibling.remove();
  }
  ...
  document.getElementById("dealerFirstCard").src =
    "PlayingCards/cardBackGreen.png";
...
function newHand() {
  ...
  document.getElementById("betText").style.left =
    10 + betTextLength * 15 + "px";
  ...
  playerTotal =
    valueParser(playerFirstCard.value, true, true) +
    valueParser(playerSecondCard.value, true, true);
  dealerTotal =
    valueParser(dealerFirstCard.value, false, true) +
    valueParser(dealerSecondCard.value, false, true);
  ...
  if (
    valueParser(playerFirstCard.value, true, false) +
      valueParser(playerSecondCard.value, true, false) ===
    21
  ) {
    document.getElementById("buttons").style.visibility = "hidden";
    document.getElementById("splitButton").style.visibility = "hidden";
  ...
  //show starting cards
  document.getElementById("playerFirstCard").src = getCardImageLink(
    playerFirstCard.suite,
    playerFirstCard.value
  );
  document.getElementById("dealerSecondCard").src = getCardImageLink(
    dealerSecondCard.suite,
    dealerSecondCard.value
  );
  document.getElementById("playerSecondCard").src = getCardImageLink(
    playerSecondCard.suite,
  ...
  document
    .getElementById("splitButton")
    .addEventListener("click", playerSplitCard);
  ...
  document
    ...
    .addEventListener("click", playerDoubleDown);
